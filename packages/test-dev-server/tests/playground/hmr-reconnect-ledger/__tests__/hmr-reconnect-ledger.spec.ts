...
// ledger resets: the next edit ships the FULL chain of factories again (the
...
describe('hmr-reconnect-ledger', () => {
  test('a reload resets the ledger: the next edit re-ships the whole chain and still hot-updates', async () => {
    ...
    try {
      ...
      // Reload: fresh clientId -> the server-side ledger for this tab resets.
      // The reload lands on the stale bundle first; the server regenerates it
      // and pushes one more reload, so wait for the fresh content to settle.
      await page.reload();
      await expect
        .poll(() => page.textContent('.chain'), { timeout: 15_000 })
        .toBe('bar(baz-v2)');
      await waitForBuildStable();
