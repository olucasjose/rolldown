...
describe('hmr-runtime-acceptance', () => {
  ...
  test('an accept that never executed (dead branch) full-reloads onto fresh content', async () => {
    ...
    // silently stale page. (The reload may bounce once while the server
    // regenerates the stale bundle, hence the generous poll.)
    await expect
      .poll(() => page.textContent('.dead'), { timeout: 15_000 })
      .toBe('dead-v2');
    await expect.poll(readMarker, { timeout: 15_000 }).toBe(null);
