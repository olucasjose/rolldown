...
// Lazy chunks are sized by the same per-client delivery ledger as HMR
...
describe('lazy-overlap-dedup', () => {
  test('the second lazy route omits the already-delivered shared factory', { retry: 0 }, async () => {
    ...
    try {
      ...
      // The ledger records a delivery when the response completes; give the
      // (async, in-process) ledger write a beat before the next compile reads it.
