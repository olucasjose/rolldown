...
/// stay stale in the ledger, so a later push re-ships or full-reloads them.
...
pub struct DevContext {
  ...
  /// Dev-engine-wide rebuild-stamp ledger of the versioned delivery protocol.
...
impl DevContext {
  ...
  pub async fn insert_pending_payload(&self, filename: String, payload: PendingPayload) {
    ...
    let mut client_entries = pending_payloads
      ...
      .collect::<Vec<_>>();
    if client_entries.len() > MAX_PENDING_PAYLOADS_PER_CLIENT {
